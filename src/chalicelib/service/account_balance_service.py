...
class AccountBalanceService:
    ...
    def add_update_account_balance(self, session, payload):
        try:
            account_balance_schema = AccountBalanceSchema.parse_obj(payload)
            account_balance = session.query(AccountBalanceORM).filter(
                AccountBalanceORM.account_id == account_balance_schema.account_id).first()
            ...
            self.return_payload['message'] = "Balance updated for account - " + \
                str(account_balance_schema.account_id)
    ...
    def get_account_balance(self, session, account_id=None):
        ...
        if self.account_id is None:
            ...
            account_balance_rows = session.query(AccountBalanceORM).filter(
                AccountBalanceORM.account_id == self.account_id)
